...
#include "../Header/TP_FlyingPawn.h"
...
ATP_FlyingPawn::ATP_FlyingPawn()
{
	...
	this->Root = CreateDefaultSubobject<UCapsuleComponent>(TEXT("Root Component"));
...
void ATP_FlyingPawn::MoveRightInput(float Val)
{
	...
	auto rot{this->PlaneMesh->GetComponentRotation()};
	UE_LOG(LogTemp, Warning, TEXT("%s"), *rot.ToString())
	// if (auto rot{this->PlaneMesh->GetComponentRotation()}; rot.Roll <= 20.f && rot.Roll >= -20.f)
	// {
	if (Val > 0.f && rot.Roll <= this->MaxRoll)
	{
		this->PlaneMesh->AddLocalRotation(FRotator{0.f, 0.f, Val});
	}
	else if (Val < 0.f && rot.Roll >= this->MinRoll)
	{
		this->PlaneMesh->AddLocalRotation(FRotator{0.f, 0.f, Val});
	};
